...
![Visual Studio のプロジェクト作成画面](./images/create-project.png)

Visual Studio で「クラス ライブラリ (.NET)」プロジェクトを作成します。フレームワークのバージョンを「.NET 10」に変更しておきます。

![ソリューション エクスプローラー](./images/solution-explorer.png)

プロジェクトが作成されたら、Koioto Plugin SDK をインポートします。ソリューション エクスプローラーで「参照」を探し、右クリックして「参照の追加...」をクリックします。

![参照マネージャー](./images/reference-manager.png)
...
![// Hello, Koioto plugging world!](./images/hello-world.png)
